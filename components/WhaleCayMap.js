...
const generateGrid = (bounds, step) => {
  const grid = [];
  const [sw, ne] = bounds;
  const latDistance = (ne[0] - sw[0]) * 111000; // approx. meters per degree latitude
  const lngDistance = (ne[1] - sw[1]) * 111000; // approx. meters per degree longitude
  ...
  const handleSquareClick = async (index) => {
    ...
    try {
      const tx = await contract.purchaseParcel(index, {
        value: ethers.parseEther("0.015") // $30 assuming 1 ETH = $2000
      });
      await tx.wait();
